...
private:
	...
	void timer_callback()
	{
		...
		tf2::Quaternion q(q_x, q_y, q_z, q_w); // Quaternion to account for angular rotation
		tf2::Matrix3x3 m(q);
		double roll, pitch, yaw;
		m.getRPY(roll, pitch, yaw); // roll pitch and yaw from rotation matrix
		...
		if (d_now < d_aim - 0.02) //2cm tolerence
		{
			double heading_error = wrap_angle(th_init - th_now);
			msg.linear.x = x_vel;
			msg.angular.z = 2.0 * heading_error; //corrects heading
		...
		else
		{
			...
			//print errors
			if (last_state_complete == 0) {
				double final_error = d_now - d_aim;
				RCLCPP_INFO(this->get_logger(), "Final Radical Error: %f", final_error);
				RCLCPP_INFO(this->get_logger(), "Final Position: x=%f y=%f", x_now, y_now);
			}
