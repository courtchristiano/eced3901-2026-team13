...
public:
    DemoNode() : Node("demo_node") {
    	...
    	"scan",
    	rclcpp::SensorDataQoS(),   // <- ensures compatibility with LiDAR
    	std::bind(&DemoNode::scan_callback, this, _1)
	);
...
private:
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        // Average 11 beams in front for safer detection
        int center = msg->ranges.size() / 2;
        double sum = 0.0;
        int count = 0;

        for (int i = -5; i <= 5; i++) {
    size_t idx = static_cast<size_t>(center + i);
    if (idx < msg->ranges.size() && std::isfinite(msg->ranges[idx])) {
        sum += msg->ranges[idx];
        count++;
    }
}
        front_distance_ = count > 0 ? sum / count : 10.0;
    }
    ...
    void timer_callback() {
        ...
        // 🚨 Obstacle avoidance
        const double SAFE_DISTANCE = 0.6;  // meters
        if (front_distance_ < SAFE_DISTANCE) {
            cmd.linear.x = 0.0;
            cmd.angular.z = 0.5; // turn left safely
